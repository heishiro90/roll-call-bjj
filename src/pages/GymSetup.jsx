...
export default function GymSetup() {
  ...
  async function createGym(e) {
    ...
    try {
      ...
      const { data: gym, error: gymErr } = await supabase
        .from('gyms')
        .insert({ name: gymName, invite_code: code, owner_id: user.id })
        .select()
        .single();
      ...
      const { error: memberErr } = await supabase
        .from('gym_members')
        .insert({ gym_id: gym.id, user_id: user.id, role: 'owner' });
  ...
  async function joinGym(e) {
    ...
    setLoading(true);
    try {
      const { data: gym, error: findErr } = await supabase
        .from('gyms')
        .select('*')
        .eq('invite_code', inviteCode.toUpperCase().trim())
        .single();
      if (findErr || !gym) throw new Error('Invalid invite code. Check with your gym owner.');

      const { error: joinErr } = await supabase
        .from('gym_members')
        .insert({ gym_id: gym.id, user_id: user.id, role: 'member' });
      if (joinErr) {
        if (joinErr.code === '23505') throw new Error('You already joined this gym!');
        throw joinErr;
      }
    ...
    <div className="page" style={{ justifyContent: 'center', alignItems: 'center', padding: 20 }}>
      <div style={{ width: '100%', maxWidth: 400 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 24, fontWeight: 400, color: '#f0ece2' }}>
            Join Your Gym
          </h2>
          <p style={{ color: 'var(--text-dim)', fontSize: 14, marginTop: 6 }}>
            Create a new gym or join one with an invite code.
          </p>
        </div>
        ...
        {!mode ? (
          ...
          <form onSubmit={joinGym} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <button type="button" className="btn btn-secondary btn-small" onClick={() => { setMode(null); setError(''); }}>
              ← Back
            </button>
          </form>
        ) : (
          <form onSubmit={createGym} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              ...
              <input
                className="input"
                type="text"
                placeholder="e.g. Gracie Barra Paris"
                value={gymName}
